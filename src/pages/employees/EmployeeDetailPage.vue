...
async function uploadPhoto(e: Event) {
  ...
  try {
    ...
    const { data } = await api.post(`/api/employees/${id}/photo`, form, {
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-center gap-3">
      <button class="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" @click="router.push('/employees')">
        ...
        <div class="flex items-start gap-5">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap items-center gap-2 mt-2">
              <span v-if="employee.position" class="text-xs font-medium px-2 py-0.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">
                {{ employee.position }}
              </span>
              <span v-if="employee.branch?.name" class="text-xs font-medium px-2 py-0.5 rounded-lg bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400">
                {{ employee.branch.name }}
              </span>
              <span v-if="employee.department?.name" class="text-xs font-medium px-2 py-0.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400">
