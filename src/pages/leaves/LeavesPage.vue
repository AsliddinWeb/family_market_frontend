...
import { Plus, RefreshCw, CheckCircle2, XCircle, Ban, Filter, Calendar, Clock } from 'lucide-vue-next'
...
// Employee faqat o'zi uchun qo'sha oladi, boshqalar uchun employee_id tanlaydi
...
function validateCreate() {
  ...
  if (createForm.value.end_date && createForm.value.start_date && createForm.value.end_date < createForm.value.start_date)
...
async function approveLeave(leave: LeaveOut) {
  const ok = await confirm({
    title: 'Tasdiqlash',
    message: `${empName(leave.employee_id)} ning ${LEAVE_TYPE_LABELS[leave.leave_type]} ta'tilini (${leave.days_count} kun) tasdiqlaysizmi?`,
    type: 'info',
  })
...
function canCancelLeave(leave: LeaveOut): boolean {
  ...
  // Employee o'zinikini bekor qila oladi
...
<template>
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-[#1a1d27] rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
      <div class="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <select v-model="filterType" class="text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-gray-700 dark:text-gray-200 focus:outline-none focus:border-primary-500">
    ...
    <AppTable :columns="columns" :rows="leaves" :loading="loading" empty-text="Ta'til so'rovlari topilmadi">
      ...
      <template #cell-days="{ row }">
        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ row.days_count }} kun</span>
      </template>
  ...
  <AppModal v-model="showCreate" title="Ta'til so'rovi" size="md">
    <div class="space-y-4">
      ...
      <!-- Xodim tanlash (faqat managerlar) -->
      <div v-if="canCreateForOthers">
        ...
        <select v-else v-model.number="createForm.employee_id"
          :class="['w-full text-sm bg-gray-50 dark:bg-gray-800 border rounded-lg px-3 py-2 text-gray-700 dark:text-gray-200 focus:outline-none focus:border-primary-500', createErrors.employee_id ? 'border-red-400' : 'border-gray-200 dark:border-gray-700']">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input v-model="createForm.start_date" type="date"
            :class="['w-full text-sm bg-gray-50 dark:bg-gray-800 border rounded-lg px-3 py-2 text-gray-700 dark:text-gray-200 focus:outline-none focus:border-primary-500', createErrors.start_date ? 'border-red-400' : 'border-gray-200 dark:border-gray-700']" />
        ...
        <div>
          ...
          <input v-model="createForm.end_date" type="date"
            :class="['w-full text-sm bg-gray-50 dark:bg-gray-800 border rounded-lg px-3 py-2 text-gray-700 dark:text-gray-200 focus:outline-none focus:border-primary-500', createErrors.end_date ? 'border-red-400' : 'border-gray-200 dark:border-gray-700']" />
          <p v-if="createErrors.end_date" class="text-xs text-red-500 mt-1">{{ createErrors.end_date }}</p>
        </div>
      </div>

      <!-- Days preview -->
      <div v-if="previewDays > 0" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 text-sm">
        <component :is="Calendar" class="w-4 h-4 shrink-0" />
        <span><strong>{{ previewDays }} kun</strong> ta'til</span>
      </div>

      <!-- Sabab -->
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Sabab <span class="text-gray-400 text-xs">(ixtiyoriy)</span></label>
