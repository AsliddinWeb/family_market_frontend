...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <template v-else>
        <StatCard :icon="Users"       label="Bugun keldi" :value="store.stats?.today_present ?? 0"  unit="ta"   color="green"  />
        <StatCard :icon="Clock"       label="Kech keldi"  :value="store.stats?.today_late ?? 0"     unit="ta"   color="amber"  />
        <StatCard :icon="UserX"       label="Kelmadi"     :value="store.stats?.today_absent ?? 0"   unit="ta"   color="red"    />
        <StatCard :icon="DollarSign"  label="Oy maoshi"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div class="lg:col-span-2"><AttendanceChart /></div>
      <div><BranchChart /></div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white dark:bg-[#1a1d27] rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
        ...
        <div v-else class="space-y-2">
          <div
            v-for="item in store.feed" :key="item.id"
          ...
          >
            <div class="flex items-center gap-3 min-w-0">
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-gray-400 truncate">{{ item.employee?.branch?.name ?? '—' }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <span class="text-xs text-gray-400">{{ item.check_in_time ? formatTime(item.check_in_time) : '—' }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white dark:bg-[#1a1d27] rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
        ...
        <div v-else class="space-y-2">
          <div v-for="sal in draftSalaries" :key="sal.id"
